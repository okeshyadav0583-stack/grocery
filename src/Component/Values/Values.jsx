    ...
    const leftValues = values .slice(0, 2).map(item =>{
        return (
            <div  key={item.Id} className='flex md:flex-row-reverse items-center gap-7'>
                <div>
                    <span className='flex justify-center items-center text-3xl text-white bg-gradient-to-b from-orange-600 w-15 h-15 rounded-full'>{item.icon}</span>
                </div>
     ...
     const rightValues = values .slice(2).map(item =>{
        return (
            <div key={item.Id} className='flex items-center gap-7 '>
                ...
                <div>
                    <h3 className='text-zinc-800md text-3xl font-bold'>{item.title}</h3>
                    <p className='text-zinc-600 mt-2'>{item.para}</p>
                </div>
            </div>
        )
